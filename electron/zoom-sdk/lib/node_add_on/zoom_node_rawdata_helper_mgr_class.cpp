...
void ZoomNodeRawDataHelperMgr::onOneWayAudioRawDataReceived(Node_AudioRawData_DEF* data_, uint32_t user_id)
{
	if (_g_audio_pipe_server._uv_ipc_server.HasClientConnected() && data_)
	{
		UVIPCMessage* rawdata_msg = MakeUVIPCMsg(data_, user_id, TYPE_AUDIO);
		_g_audio_pipe_server._uv_ipc_server.SendMessage(rawdata_msg);
	}
}
void ZoomNodeRawDataHelperMgr::onShareAudioRawDataReceived(Node_AudioRawData_DEF* data_, uint32_t user_id)
{

}
void ZoomNodeRawDataHelperMgr::onOneWayInterpreterAudioRawDataReceived(Node_AudioRawData_DEF* data_, ZoomSTRING pLanguageName)
{
