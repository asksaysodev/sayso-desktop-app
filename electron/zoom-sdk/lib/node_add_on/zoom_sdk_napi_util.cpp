...
extern "C" NODE_MODULE_EXPORT void CallbackOnewayAudioFormatDataToJS(int32_t nodeid, const std::string & audio_data_format,
	char* audio_data_buf, int len, v8::Local<v8::Function>& executor, fnFreeDataBufferNotification notification, void* hint)
{
	do
	{
		...
		status = napi_create_int32(g_napi_env, nodeid, &napi_nodeid);
		if (napi_ok != status)
			break;
