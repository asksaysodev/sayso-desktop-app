...
ZNSDKError ZMeetingAudioWrap::EnableMuteOnEntry(bool bEnable, bool allow)
{
    ZoomSDKSettingService *setting = [[ZoomSDK sharedSDK] getSettingService];
    if (setting) {
        ZoomSDKGeneralSetting *general = [setting getGeneralSetting];
        if (general) {
            ZoomSDKError ret = [general enableMuteOnEntry:bEnable allowUnmuteBySelf:allow];
            nativeErrorTypeHelp  Help_type;
            return Help_type.ZoomSDKErrorType(ret);
        }
    }
...
bool ZMeetingAudioWrap::CanEnableMuteOnEntry()
{
    ...
    return [actionController canEnableMuteOnEntry];
}
...
bool ZMeetingAudioWrap::IsMuteOnEntryEnabled()
{
    ...
    return [actionController isMuteOnEntryEnabled];
}

ZNSDKError ZMeetingAudioWrap::EnablePlayMeetingAudio(bool bEnabled)
{
    ZoomSDKMeetingService *service = [[ZoomSDK sharedSDK] getMeetingService];
    if (!service) {
        return ZNSDKERR_SERVICE_FAILED;
    ...
    ZoomSDKError ret = [actionController enablePlayMeetingAudio:bEnabled];
    nativeErrorTypeHelp Help_type;
    return Help_type.ZoomSDKErrorType(ret);
}
...
bool ZMeetingAudioWrap::IsPlayMeetingAudioEnabled()
{
    ...
    return [actionController isPlayMeetingAudioEnabled];
}
...
void ZMeetingAudioWrap::onMuteOnEntryStatusChange(bool enable)
{
    if (m_pSink) {
        m_pSink->onMuteOnEntryStatusChange(enable);
    }
}