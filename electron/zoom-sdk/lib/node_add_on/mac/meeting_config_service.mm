...
ZNSDKError ZMeetingConfigWrap::EnableInviteButtonOnMeetingUI(bool bEnable)
{
    ...
    if (service && config) {
        config.hideInviteButtonOnHCWindow = (bEnable == YES) ? NO : YES;
    }
...
void ZMeetingConfigWrap::SetShowCallInTab(bool bShow)
{
    ...
    if (service) {
        ...
        if (config) {
            ...
            config.hideTelephoneInAudioWindow = isHide;
        }
    }
}
...
void ZMeetingConfigWrap::RedirectMeetingInputUserInfoDialog(bool hide)
{
    ZoomSDKMeetingService *service = [[ZoomSDK sharedSDK] getMeetingService];
    if (service) {
        ZoomSDKMeetingConfiguration *config = [service getMeetingConfiguration];
        if (config) {
            [config setHideMeetingInputUserInfoWindow:hide];
        }
    }
}

void ZMeetingConfigWrap::RedirectConfirmStartArchiveDialog(bool hide)
{
    ZoomSDKMeetingService *service = [[ZoomSDK sharedSDK] getMeetingService];
    if (service) {
        ZoomSDKMeetingConfiguration *config = [service getMeetingConfiguration];
        if (config) {
            config.hideConfirmStartArchiveWindow = hide;
        }
    }
}
...
{
    ZoomSDKJoinMeetingHelper *helper = [meetingServiceDelegate share].joinMeetingHelper;
    if (helper) {
...
{
    ZoomSDKWebinarRegisterHelper *helper = [WebinarControllerDelegate share].webinarRegisterHelper;
    if (!helper) {
...
ZNSDKError ZMeetingConfigWrap::EnableZoomDocs(bool enable)
{
    ...
    ZoomSDKError ret = [shareScreen enableZoomDocs:enable];
    nativeErrorTypeHelp help;
    return help.ZoomSDKErrorType(ret);
}
...
{
    ZoomSDKWebinarInputScreenNameHandler *handler = [WebinarControllerDelegate share].webinarInputScreenNameHandler;
    if (handler) {
...
void ZMeetingConfigWrap::onJoinMeetingNeedUserInfo()
{
    if (m_pSink) {
        m_pSink->onJoinMeetingNeedUserInfo();
    }
}

void ZMeetingConfigWrap::onUserConfirmToStartArchive()
{
    if (m_pSink) {
        m_pSink->onUserConfirmToStartArchive();
    }
}

ZoomSTRING ZMeetingConfigWrap::GetDefaultDisplayName()
{
    ZoomSDKMeetingInputUserInfoHandler *handler = [meetingServiceDelegate share].meetingInputUserInfoHandler;
    if (!handler)
        return "";

    NSString *displayName = [handler defaultDisplayName];
    if (!displayName)
        return "";
...
bool ZMeetingConfigWrap::CanModifyDefaultDisplayName()
{
    ZoomSDKMeetingInputUserInfoHandler *handler = [meetingServiceDelegate share].meetingInputUserInfoHandler;
    if (!handler)
        return false;
    
    return [handler canModifyDefaultDisplayName];
}

bool ZMeetingConfigWrap::IsValidEmail(ZoomSTRING zn_email)
{
    ZoomSDKMeetingInputUserInfoHandler *handler = [meetingServiceDelegate share].meetingInputUserInfoHandler;
    if (!handler)
        return false;
    ...
    NSString *email = [NSString stringWithUTF8String:zn_email.c_str()];
    return [handler isValidEmail:email];
}

ZNSDKError ZMeetingConfigWrap::InputUserInfo(ZoomSTRING zn_name, ZoomSTRING zn_email)
{
    ZoomSDKMeetingInputUserInfoHandler *handler = [meetingServiceDelegate share].meetingInputUserInfoHandler;
    if (!handler)
        return ZNSDKERR_WRONG_USEAGE;

    if (zn_name.empty() || zn_email.empty())
    {
        return ZNSDKERR_INVALID_PARAMETER;
    }

    NSString *name = [NSString stringWithUTF8String:zn_name.c_str()];
    NSString *email_str = [NSString stringWithUTF8String:zn_email.c_str()];
    ZoomSDKError ret = [handler inputUserInfo:name email:email_str];
    nativeErrorTypeHelp Help_type;
    return Help_type.ZoomSDKErrorType(ret);
}

ZNSDKError ZMeetingConfigWrap::CancelInputUserInfo()
{
    ZoomSDKMeetingInputUserInfoHandler *handler = [meetingServiceDelegate share].meetingInputUserInfoHandler;
    if (!handler)
        return ZNSDKERR_WRONG_USEAGE;
...
ZoomSTRING ZMeetingConfigWrap::GetArchiveConfirmContent()
{
    ZoomSDKMeetingArchiveConfirmHandler *handler = [meetingServiceDelegate share].meetingArchiveConfirmHandler;
    if (!handler)
        return "";

    NSString *confirmContent = handler.archiveConfirmContent;
    if (!confirmContent)
        return "";

    return confirmContent.UTF8String;
}

ZNSDKError ZMeetingConfigWrap::JoinWithArchive(bool startArchive)
{
    ZoomSDKMeetingArchiveConfirmHandler *handler = [meetingServiceDelegate share].meetingArchiveConfirmHandler;
    if (!handler)
        return ZNSDKERR_WRONG_USEAGE;

    ZoomSDKError ret = [handler joinWithArchive:startArchive];
    nativeErrorTypeHelp Help_type;
    return Help_type.ZoomSDKErrorType(ret);
}
