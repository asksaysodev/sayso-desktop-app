...
{
    m_pSink = 0;
    ZoomSDKRealNameAuthenticationController *controller = [[[ZoomSDK sharedSDK] getMeetingService] getRealNameController];
    if(controller)
    {
...
void ZSDKSMSHelperWrap::SetSink(ZNativeSDKSMSHelperWrapSink *pSink)
{
    ...
    if(controller)
    {
        controller.delegate = [realNameAuthenticationDelegate share];
    }
}
...
{
    meetingServiceDelegate *service = [ZoomSDKVerifySMSVerificationCodeController share];
    if (!service) {
        return false;
    }
...
{
    ZoomSDKRetrieveSMSVerificationCodeController *controller = [realNameAuthenticationDelegate share].retriveController;
    if (!controller) {
        return false;
    }
    ...
    if (country_code.length() > 0 && phoneNum.length > 0) {
        ZoomSDKError ret = [controller retrieveWithCountryCode:countryCode PhoneNum:phoneNum];
        if (ret == ZoomSDKError_Success) {
            return true;
        }
        return false;
...
{
    ZoomSDKVerifySMSVerificationCodeController *controller = [realNameAuthenticationDelegate share].verifyCintroller;
    if (!controller) {
        return false;
    }
    NSString *countryCode = [NSString stringWithCString:country_code.c_str() encoding:NSUTF8StringEncoding];
