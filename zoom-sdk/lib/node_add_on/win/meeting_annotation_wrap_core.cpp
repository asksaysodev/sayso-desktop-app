...
{
	SDKEventWrapMgr::GetInst().m_meetingAnnotationCtrlWrapEvent.SetOwner(this);
	m_pSink = 0;
}
ZMeetingAnnotationWrap::~ZMeetingAnnotationWrap()
{
	Uninit();
	m_pSink = 0;
	SDKEventWrapMgr::GetInst().m_meetingAnnotationCtrlWrapEvent.SetOwner(NULL);
}
void ZMeetingAnnotationWrap::Init()
{
	...
	ZOOM_SDK_NAMESPACE::CSDKWrap::GetInst().GetMeetingServiceWrap().T_GetAnnotationController().SetEvent(&SDKEventWrapMgr::GetInst().m_meetingAnnotationCtrlWrapEvent);
}
void ZMeetingAnnotationWrap::Uninit()
{
	ZOOM_SDK_NAMESPACE::CSDKWrap::GetInst().GetMeetingServiceWrap().T_GetAnnotationController().SetEvent(NULL);
	ZOOM_SDK_NAMESPACE::CSDKWrap::GetInst().GetMeetingServiceWrap().T_GetAnnotationController().Uninit_Wrap();
}
void ZMeetingAnnotationWrap::SetSink(ZNativeSDKMeetingAnnotationWrapSink* pSink)
{
	m_pSink = pSink;
}
...
void ZMeetingAnnotationWrap::onSupportAnnotationStatus(unsigned int nShareSourceID, bool bSupportAnnotation)
{
	if (m_pSink)
	{
		m_pSink->onSupportAnnotationStatus(nShareSourceID, bSupportAnnotation);
	}
}