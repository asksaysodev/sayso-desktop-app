...
void ZoomNodeMeetingH323CtrlWrap::GetCalloutH323DeviceList(const v8::FunctionCallbackInfo<v8::Value>& args)
{
	v8::Isolate* isolate = args.GetIsolate();
	auto context = isolate->GetCurrentContext();
	ZNList<ZNH323DeviecInfo> h323DeviceList = _g_native_wrap.GetMeetingServiceWrap().GetMeetingH323Ctrl().GetCalloutH323DeviceList();
	v8::Local<v8::Array> nodes = v8::Array::New(isolate);
...
void ZoomNodeMeetingH323CtrlWrap::SetH323CallOutStatusCB(const v8::FunctionCallbackInfo<v8::Value>& args)
{
	...
	do
	{
		...
		ZoomNodeSinkHelper::GetInst().onH323CalloutStatusNotify.Reset(isolate, cbfunc);
