...
bool ZSettingAudioWrap::IsAutoJoinAudioEnabled()
{
    ...
    ZoomSDKAudioSetting *audio = [service getAudioSetting];
    if(audio){
        return [audio isJoinAudioWhenJoinMeetingOn];
    }
    return false;
}
...
ZNSDKError ZSettingAudioWrap::SetEchoCancellationLevel(ZN_SDK_ECHO_CANCELLATION_LEVEL level)
{
    ...
    switch (level) {
        ...
        case ZN_SDK_ECHO_CANCELLATION_LOW:
            ret = ZoomSDKAudioEchoCancellationLevel_Low;
            break;
        case ZN_SDK_ECHO_CANCELLATION_HIGH:
            ret = ZoomSDKAudioEchoCancellationLevel_High;
            break;
        default:
            break;
    }
...
ZN_SDK_ECHO_CANCELLATION_LEVEL ZSettingAudioWrap::GetEchoCancellationLevel()
{
    ...
    switch (level) {
        case ZoomSDKAudioEchoCancellationLevel_Auto:
            return ZN_SDK_ECHO_CANCELLATION_DEFAULT;
        case ZoomSDKAudioEchoCancellationLevel_Low:
            return ZN_SDK_ECHO_CANCELLATION_LOW;
        case ZoomSDKAudioEchoCancellationLevel_High:
            return ZN_SDK_ECHO_CANCELLATION_HIGH;
    }
}
//callback
