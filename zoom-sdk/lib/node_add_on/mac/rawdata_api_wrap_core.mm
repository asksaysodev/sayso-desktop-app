...
ZNSDKError ZNativeRawAPIWrap::CreateRenderer(unsigned long long recv_handle)
{
    ...
    ZoomSDKError ret = [rawdataCtrl createRender:&videoRender];
    if (ret == ZoomSDKError_Success) {
        if (videoRender) {
...
ZNSDKError ZNativeRawAPIWrap::DestroyRenderer(unsigned long long recv_handle)
{
    ...
    for (NSNumber *num in arr) {
        ...
        if(handle == recv_handle){
            ...
            ZoomSDKError ret = [rawdataCtrl destroyRender:renderClass.render];
            if(ret == ZoomSDKError_Success){
...
ZNSDKError ZNativeRawAPIWrap::Subscribe(uint32_t subscribeId, ZNZoomSDKRawDataType type, unsigned long long recv_handle)
{
    NSArray *arr = renderDic.allKeys;
    for (NSNumber *num in arr) {
        unsigned long long handle = [num unsignedLongLongValue];
        if(handle == recv_handle){
            ...
            if (render){
                ZoomSDKError ret = [render subscribe:subscribeId rawDataType:ZoomMapRawDataType(type)];
                if (ret == ZoomSDKError_Success){
...
ZNSDKError ZNativeRawAPIWrap::GetSubscribeId(uint32_t& subscribeId, unsigned long long recv_handle)
{
    NSArray *arr = renderDic.allKeys;
    for (NSNumber *num in arr) {
        unsigned long long handle = [num unsignedLongLongValue];
        if(handle == recv_handle){
            ...
            ZoomSDKRenderer *render = renderClass.render;
            if (render){
                subscribeId = [render getSubscribeID];
                return ZNSDKERR_SUCCESS;
            }
        }
    }
