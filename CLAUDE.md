...
- Known Windows-only seams: `Menu.setApplicationMenu(null)` (its menu is in-window, not a system menu bar) plus the `before-input-event` DevTools shortcut that replaces the F12 binding it took with it, Window Controls Overlay in `WindowManager.getTitleBarConfig()`, `skipTaskbar` on the two overlays, the tray menu's transparent background with its surface drawn in CSS, and the `app.setAppUserModelId(build.appId)` call at module scope in `main.ts` (packaged builds only) that names the Launch at Login registry value — with `electron/utils/launchAtLogin.ts` migrating pre-AUMID entries and `assets/installer.nsh` deleting them on uninstall. `docs/IPC_CONTRACT.md` has the full table.
