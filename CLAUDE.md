...
- **Why they sit next to the source:** `tsconfig.electron.json` sets `outDir` *and* `rootDir` to `electron`, so `tsc` emits each file beside its `.ts` rather than into a separate `dist/`; `sourceMap: true` adds the `.js.map`. Node can't require a `.ts`, so the emitted `.js` is what actually runs. Expect a `.js` + `.js.map` pair for every `.ts` in `electron/` — that's normal, not stray build junk.
...
- The `.gitignore` rules are **per-directory**, one pair of lines per TS folder (`electron/utils/*.js` + `*.js.map`, and the same for `audio/`, `auth/`, `permissions/`, `shared/`, plus `electron/*.js` at the top level). Consequence: **`electron/streaming/` has no ignore rule at all**, deliberately, because its `.js` files are the hand-written ones. So adding a `.ts` to `electron/streaming/` emits an *untracked* `.js` that is indistinguishable from the tracked hand-written ones — easy to commit by accident, after which the stale committed `.js` is what Electron loads. New files in `electron/streaming/` therefore stay hand-written CommonJS `.js`, matching the rest of the folder. If you do add a `.ts` there, add **exact-path** ignore lines for its `.js` and `.js.map` — never an `electron/streaming/*.js` glob, which would silently swallow the next hand-written file someone adds.
...
- `electron/shared/` is the one place both TypeScript programs read from (`src → electron` works; `electron → src` is a hard TS6059 because `rootDir` is pinned). Its files must be **import-free and side-effect-free** — that, not "types only", is the invariant: nothing node- or electron-shaped may reach the six renderer bundles. `redact.ts` is runtime code and complies. Two consequences: write **ES2020-safe syntax only** (`Object.hasOwn`, `.at()`, `.replaceAll()` compile under `tsconfig.electron.json`'s ES2022 and *fail* the renderer's ES2020), and if the renderer imports one of these modules for its **values**, add a `resolve.alias` entry in `vite.config.ts` pinning the `.ts` — tsc emits a `.js` next to it and Vite resolves `.js` first, silently picking up CommonJS that rollup can't tree-shake.
...
- Buffer first, convert after — the converter uses the `format` the native module reports, so a native module must describe its buffers **truthfully** rather than match a fixed rate. `docs/NATIVE_AUDIO_CONTRACT.md` is the frozen NAN surface (13 required methods plus optional ones like `getMicInputDiagnostics`, which `index.js` feature-checks); `electron/native-audio/index.js` is the authority on how those methods are called — implement to match it, don't change it to fit a new native shape.
...
- Known Windows-only seams: `Menu.setApplicationMenu(null)` (its menu is in-window, not a system menu bar) plus the `before-input-event` DevTools shortcut that replaces the F12 binding it took with it, Window Controls Overlay in `WindowManager.getTitleBarConfig()`, `skipTaskbar` on the two overlays, the tray menu's transparent background with its surface drawn in CSS, and the `app.setAppUserModelId()` call at module scope in `main.ts` (packaged builds only) that names the Launch at Login registry value — sourced from a top-level `app_id` in the packaged `package.json`, never `build.appId`, which electron-builder strips — with `electron/utils/launchAtLogin.ts` migrating pre-AUMID entries and `assets/installer.nsh` deleting them on uninstall. `docs/IPC_CONTRACT.md` has the full table.
...
Release flow (`docs/VERSIONING.md`): there is no `main` branch. `feature/* → development → npm version <patch|minor|major> → staging`. **Bump the version on `development`, never on `staging`** — doing it backwards is what shipped a mislabeled 1.2.5. Both staging and production artifacts build from `staging`; `fresh-export` aborts if local `package.json` disagrees with `origin/staging`. Generate release notes with the `sayso-release-notes` skill (`.claude/skills/`) and set them as the draft's description before publishing. Step-by-step: `docs/PUBLISH_RELEASE.md`.
...
- **Logging a raw error object is safe, and is the intended style.** Redaction lives at the serialization boundary — `formatLogArgs` in `setupLogging()`, plus `beforeSend`/`beforeBreadcrumb` in both Sentry configs, all from `electron/shared/redact.ts` (SAYSO-460: an `AxiosError`'s `toJSON()` put `headers.Authorization` on disk in plaintext). Don't "fix" this per call site, and don't hand-pick `err.message` to be safe — that just loses the stack. The `console.warn` in `loadBootSession` deliberately logs a raw `AxiosError` as the regression test.
- `<userData>/logs/` is load-bearing beyond logging: `electron/utils/permissionsMigration.ts` reads the **directory's existence** as the macOS "prior install" signal. Never remove the directory — `electron/utils/logRetention.ts` deletes individual `sayso-*.log` files only, never `main.log` (electron-log holds it open) and never `recursive: true`.
