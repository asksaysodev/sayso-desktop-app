...
5. **"My Account was disabled while reconnecting."** Disabling it was wrong and was removed. The row stays clickable in every state, and is only disabled while its own request is in flight. It does now make one request, for a session handoff token (SAYSO-433), but that lives in main (`auth:open-web-app`) and is *skipped* outright while `global.networkState` is `reconnecting` rather than attempted and failed. The page opens either way, just at the login screen.
