...
| `src/trayMenu/TrayMenuApp.tsx` | Opens the web app signed in when the user clicks "My Account". Calls `POST /auth/desktop-handoff` for a single-use token and opens `app.asksayso.com/#token_hash=...`. It no longer sends `access_token`: the web app cannot build a session from one without a refresh token, and supabase-js reads such a hash as a failed implicit-grant callback and clears the browser's existing session (SAYSO-433). The server returns `token_hash: null` while a password reset is pending — the two share one Supabase token slot — and the page then opens at the login screen. That window normally ends as soon as the reset link is opened, not an hour later: the web app clears the flag the moment the link verifies. |
