...
| `src/trayMenu/TrayMenuApp.tsx` | Invokes `auth:open-web-app` when the user clicks "My Account"; main does the rest. Main fetches a single-use token from `POST /auth/desktop-handoff` and opens `app.asksayso.com/#token_hash=...`. It no longer sends `access_token`: the web app cannot build a session from one without a refresh token, and supabase-js reads such a hash as a failed implicit-grant callback and clears the browser existing session (SAYSO-433). Living in main keeps the token out of the renderer and means the live session is read at request time, so a sign-out mid-flight cannot open a signed-in browser. The server returns `token_hash: null` while a password reset is pending, since the two share one Supabase token slot, and the page then opens at the login screen; that window ends as soon as the reset link is verified, not an hour later. An account with MFA enrolled lands on `/mfa-verify` first, because a redeemed magic-link session is AAL1. |
