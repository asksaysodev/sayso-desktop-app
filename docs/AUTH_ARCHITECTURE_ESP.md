...
| `src/trayMenu/TrayMenuApp.tsx` | Abre la web app con sesión iniciada cuando el usuario hace click en "My Account". Llama a `POST /auth/desktop-handoff` por un token de un solo uso y abre `app.asksayso.com/#token_hash=...`. Ya no envía `access_token`: la web app no puede construir una sesión sin un refresh token, y supabase-js interpreta ese hash como un implicit-grant callback fallido y borra la sesión que el navegador ya tenía (SAYSO-433). El servidor devuelve `token_hash: null` mientras hay un reseteo de contraseña pendiente — ambos comparten un único slot de token en Supabase — y la página abre en la pantalla de login. Esa ventana normalmente termina apenas se abre el link de reseteo, no una hora después: la web app limpia el flag en cuanto el link se verifica. |
