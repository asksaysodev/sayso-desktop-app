...
| `src/trayMenu/TrayMenuApp.tsx` | Invoca `auth:open-web-app` cuando el usuario hace click en "My Account"; el resto lo hace main. Main pide un token de un solo uso a `POST /auth/desktop-handoff` y abre `app.asksayso.com/#token_hash=...`. Ya no envía `access_token`: la web app no puede construir una sesión sin un refresh token, y supabase-js interpreta ese hash como un implicit-grant callback fallido y borra la sesión que el navegador ya tenía (SAYSO-433). Vivir en main mantiene el token fuera del renderer y lee la sesión viva al momento del request, así un sign-out a mitad de vuelo no puede abrir un navegador con sesión iniciada. El servidor devuelve `token_hash: null` mientras hay un reseteo de contraseña pendiente, porque ambos comparten un único slot de token en Supabase, y la página abre en la pantalla de login; esa ventana termina apenas se verifica el link, no una hora después. Una cuenta con MFA cae primero en `/mfa-verify`, porque una sesión de magic link es AAL1. |
