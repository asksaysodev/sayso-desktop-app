...
> pins that to `build.appId` at module scope (packaged builds only — see
...
| `app.setAppUserModelId(build.appId)` at module scope (~78) | win32-only, packaged only | Windows identity string. Drives taskbar grouping, toast identity and — the reason it is set — the **value name** Electron writes for Launch at Login. Unset, Electron derives `electron.app.<app.name>`, matching neither `build.appId` nor the AUMID electron-builder stamps on the shortcuts. Read from `package.json` `build.appId`, which `build.staging.js` overrides through `extraMetadata` so both channels are correct. **Module scope, not `whenReady()`:** it must precede any window, tray or login-item call, and `whenReady()` early-returns behind the OS-version and `/Applications` guards. Skipped when unpackaged so a dev toggle can't repoint the installed app's Run value at the dev binary. |
